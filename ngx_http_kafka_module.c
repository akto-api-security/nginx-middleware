...
static void ngx_http_kafka_post_callback_handler(ngx_http_request_t *r)
{
    ...
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "return 6");


    ngx_log_error(NGX_LOG_ERR, conn_log, 0, "sending data");
    ngx_log_error(NGX_LOG_ERR, conn_log, 0, (const char *)local_conf->topic.data);
    ngx_log_error(NGX_LOG_ERR, conn_log, 0, "sending datai done");

    len = strlen((const char *)((r->uri).data));
    rc = rd_kafka_produce(local_conf->rkt, (int32_t)local_conf->partition,
            RD_KAFKA_MSG_F_COPY, (void *)((r->uri).data), len, NULL, 0, conn_log);


    rd_kafka_conf_set(main_conf->rkc,"bootstrap.servers", "127.0.0.1:9092", NULL, 0);
